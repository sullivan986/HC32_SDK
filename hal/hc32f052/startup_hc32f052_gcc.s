@*******************************************************************************
@  Startup / vector table for XHSC HC32F052 (ARM Cortex-M0+, ARMv6-M)
@
@  This file is written in GNU assembler syntax and assembles with BOTH:
@    - arm-none-eabi-gcc   (GNU as)
@    - ATfE clang          (LLVM integrated assembler)
@
@  Link it with -nostartfiles together with the matching linker script
@  hc32f052.ld, e.g. (GCC):
@    arm-none-eabi-gcc -mcpu=cortex-m0plus -mthumb -nostartfiles \
@        -T hc32f052.ld startup_hc32f052_gcc.s ... -o app.elf
@  or (ATfE):
@    clang --target=arm-none-eabi -mcpu=cortex-m0plus -mthumb \
@        -nostartfiles -T hc32f052.ld startup_hc32f052_gcc.s ... -o app.elf
@
@  Symbol contract (all provided by hc32f052.ld):
...
__Vectors:
    ...
    .word   VC0_1_LVD_IRQHandler        @ IRQ #26
    .word   Dummy_IRQHandler            @ IRQ #27
    .word   CAN_IRQHandler              @ IRQ #28
    .word   ATIM_BK_IRQHandler          @ IRQ #29
    .word   FLASH_RAM_IRQHandler        @ IRQ #30
    .word   CTRIM_FAULT_IRQHandler      @ IRQ #31
...
@===============================================================================
    ...
    .thumb_func
Reset_Handler:
    @ --- On Cortex-M0+ VTOR defaults to 0x00000000, which is where the vector
    @ --- table lives, so no VTOR setup is required for flash execution. ---
...
.Lzero_check:
    ...
    @ --- System clock / device early init (provided by system_hc32f052.c) ---
    bl      SystemInit

    @ --- Run C/C++ constructors (__init_array). Provided by newlib AND picolibc ---
